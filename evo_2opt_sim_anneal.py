...
class Combined_Evo_TwoOpt(Strategy):
	...
	def evolve(self,population,pop_size,top_k,step,p=None):
		...
		if step % 500 == 0:  #Every 100 steps, apply two-opt
			ranked_pop = [(self.iterate_on_2optSwap(p,iterations=2,stop_if_no_progress=True,t_0=6000/(step+1))[0],s) for p,s in ranked_pop]
	...
	def iterate_on_2optSwap(self,truck_paths,iterations=1,stop_if_no_progress=True,t_0=10):
