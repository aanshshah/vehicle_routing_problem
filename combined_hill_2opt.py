...
class Combined_Hill2OPT_TwoOpt(Strategy):
	...
	def iterate_on_solution(self,truck_paths,selector,iterations=1000,stop_if_no_progress=False):
			...
			for step in range(iterations):
				...
				if step % 500 == 0:
					solution,objective_value = self.iterate_on_2optSwap(self.flatten(solution),iterations=10,stop_if_no_progress=True)
					solution = self.unflatten(solution)


				if step % 10 == 0: print("step: {}, cost: {}".format(step,objective_value))
